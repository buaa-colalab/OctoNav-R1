...
from habitat.tasks.nav.octonav_task import (ExtendedInstructionData, OctoGoal,
                                            OctoNavEpisode)
...
class OctoNavDatasetV1(Dataset):
    ...
    def check_config_paths_exist(config: "DictConfig") -> bool:
        ...
        for dataset in config.datasets:
            flag = (flag and os.path.exists(
                dataset.data_path.format(split=dataset.split))
                    and os.path.exists(dataset.scenes_dir))
        return flag
    ...
    def get_scenes_to_load(cls, config: "DictConfig") -> List[str]:
        ...
        dataset = cls(config)
        return sorted(
            {cls.scene_from_scene_path(e.scene_id)
             for e in dataset.episodes})

    @staticmethod
    def _get_scenes_from_folder(content_scenes_path: str,
            ...
            if "VLN-CE" in dataset.task:
                if dataset.task == "RxR-VLN-CE":
                    for role in dataset.roles:
                        datasetfile_path = dataset.data_path.format(
                            split=dataset.split, role=role)
                    ...
                    if ALL_LANGUAGES_MASK not in dataset.languages:
                        ...
                        self.episodes = [
                            episode for episode in self.episodes if
                            episode.instruction.language in languages_to_load
                        ]
                else:
                    datasetfile_path = dataset.data_path.format(
                        split=dataset.split)
                ...
                self.episodes = list(
                    filter(self.build_content_scenes_filter(config),
                           self.episodes))
            else:
                datasetfile_path = dataset.data_path.format(
                    split=dataset.split)
                ...
                if has_individual_scene_files:
                    ...
                    if ALL_SCENES_MASK in scenes:
                        ...
                        scenes = [
                            scene for scene in scenes if scene in cur_scenes
                        ]
    ...
    def from_json(
        ...
        if task == "ObjectNav":
            if CONTENT_SCENES_PATH_FIELD in deserialized:
                self.content_scenes_path = deserialized[
            ...
            if "category_to_scene_annotation_category_id" in deserialized:
                self.category_to_scene_annotation_category_id[dataset_name] = (
                    deserialized["category_to_scene_annotation_category_id"])

            if "category_to_mp3d_category_id" in deserialized:
                self.category_to_scene_annotation_category_id[dataset_name] = (
                    deserialized["category_to_mp3d_category_id"])

            assert len(self.category_to_task_category_id[dataset_name]) == len(
                self.category_to_scene_annotation_category_id[dataset_name])

            assert set(self.category_to_task_category_id[dataset_name].keys(
            )) == set(
                self.category_to_scene_annotation_category_id[dataset_name].
                keys()
            ...
            for k, v in deserialized["goals_by_category"].items():
                self.goals_by_category[dataset_name][k] = [
                    self.__deserialize_goal(g) for g in v
                ]

            for i, episode in enumerate(deserialized["episodes"]):
                episode = OctoNavEpisode(task_name=task,
                ...
                if scenes_dir is not None:
                    ...
                    episode.scene_id = os.path.join(scenes_dir,
                                                    episode.scene_id)

                episode.goals = self.goals_by_category[dataset_name][
                    episode.goals_key]
            ...
            for i, origin_episodes in enumerate(deserialized["episodes"]):
                ...
                for item in origin_episodes["task_episodes"]:
                    episode.task_episodes.append(
                        task_episodes[item["task"]][item["id"]])
                episode.shortest_path = origin_episodes["info"][
                    "shortest_path"]
    ...
    def __deserialize_goal(serialized_goal: Dict[str, Any]) -> OctoGoal:
        ...
        if "image_goals" in serialized_goal:
            for iidx, params in enumerate(g.image_goals):
                g.image_goals[iidx] = InstanceImageParameters(
