...
class EnvBatchRenderer:
    ...
    def __init__(self, config: DictConfig, num_envs: int) -> None:
        ...
        assert (config.habitat.simulator.habitat_sim_v0.enable_gfx_replay_save
                ), ("Batch renderer requires enable_gfx_replay_save "
                    "to be enabled in config.")
        assert not config.habitat.simulator.create_renderer, (
            "Batch renderer requires create_renderer to be disabled in config."
        )
        assert not config.habitat.simulator.debug_render, (
            "Batch renderer requires debug_render to be disabled in config.")
        logger.warn("Batch rendering enabled. This feature is "
        ...
        assert len(self._sensor_specifications) == 1, (
            "Batch renderer only supports one sensor but configuration "
            "specifies {len(self._sensor_specifications)} sensors. "
            "Either remove sensors or disable batch rendering.")
        ...
        replay_renderer_creation_fn: Callable[
            [ReplayRendererConfiguration], ReplayRenderer] = (
                ReplayRenderer.create_batch_replay_renderer if
                not config.habitat.simulator.renderer.classic_replay_renderer
    ...
    def close(self) -> None:
        ...
        """
        if self._replay_renderer is not None:
    ...
    def post_step(self, observations: List[OrderedDict]) -> List[OrderedDict]:
        ...
        for env_index in range(self._num_envs):
            ...
            assert KEYFRAME_OBSERVATION_KEY in env_obs, (
                "Keyframe missing from environment "
                "observations. "
                "Batch rendering cannot proceed.")
    ...
    def _draw_observations(
            self,
            sensor_spec: BackendSensorSpec) -> Union[np.ndarray, "Tensor"]:
        r"""
        Draw observations for all environments.

        :param sensor_spec: Habitat-sim sensor
        specifications.
        :return: A numpy ndarray in GPU-to-CPU mode,
        or a torch tensor in GPU-to-GPU mode.
        """
        draw_fn: Callable = (self._draw_observations_gpu_to_gpu
                             if self._gpu_gpu else
                             self._draw_observations_gpu_to_cpu)
        return draw_fn(sensor_spec)

    def _draw_observations_gpu_to_cpu(
            self, sensor_spec: BackendSensorSpec) -> np.ndarray:
        r"""
        Draw observations for all environments.
        Copies sensors output from GPU memory into CPU ndarrays,
         during which the thread is blocked.
    ...
    def _draw_observations_gpu_to_gpu(
            self, sensor_spec: BackendSensorSpec) -> "Tensor":
        raise NotImplementedError

    def copy_output_to_image(self) -> List[np.ndarray]:
        r"""
        Utility function that creates a list of RGB images
        (as ndarrays) for each
    ...
    @staticmethod
    def _create_core_sensor_suite(config: DictConfig, ) -> CoreSensorSuite:
        r"""
        Instantiates a core sensor suite from configuration
        that only contains visual sensors.
    ...
    def _create_sensor_specifications(
            config: DictConfig,
            sensor_suite: CoreSensorSuite) -> List[BackendSensorSpec]:
        ...
        for sensor in sensor_suite.sensors.values():
            ...
            if sim_sensor_cfg.noise_model != "None":
                logger.warn("The batch renderer currently doesn't support "
                            "sensor noise modeling. Noise model for sensor '" +
                            sim_sensor_cfg.uuid + "' will be ignored.")
    ...
    def _create_replay_renderer_cfg(
        ...
        :param sensor_specifications: Habitat-Sim visual sensor
        specifications. See _create_sensor_specifications().
    ...
    @staticmethod
    def _load_composite_files(config: DictConfig,
                              replay_renderer: ReplayRenderer):
        # Pre-load graphics assets using composite GLTF files.
        ...
        if config.habitat.simulator.renderer.composite_files is not None:
            for (composite_file
                ...
                if os.path.isfile(composite_file):
                    logger.info("Pre-loading composite file: " +
                                composite_file)
                    replay_renderer.preload_file(composite_file)
                    loaded_composite_file_count += 1
                else:
                    logger.error("Failed to load composite file: " +
                                 composite_file)
        if loaded_composite_file_count == 0:
            logger.warn("No composite file was pre-loaded. "
                        "Expect lower batch rendering performance.")
    ...
    def _float_image_to_rgb_image(float_image: np.ndarray):
        ...
        for y, x in np.ndindex(float_image.shape):
            ...
            normalized_color_value = np.uint8(
                255.0 * ((distance_from_camera - float_min) /
                         (float_max - float_min)))
