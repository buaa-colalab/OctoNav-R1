...
def angle_between_quaternions(q1: quaternion.quaternion,
                              q2: quaternion.quaternion) -> float:
...
def quaternion_from_two_vectors(v0: np.ndarray,
                                v1: np.ndarray) -> quaternion.quaternion:
...
def quaternion_rotate_vector(quat: quaternion.quaternion,
...
def agent_state_target2ref(
    ...
    assert len(
        ref_agent_state[1]) == 3, ("Only support Cartesian format currently.")
    assert len(target_agent_state[1]) == 3, (
...
def random_triangle_point(v0: np.ndarray, v1: np.ndarray,
...
def is_point_in_triangle(p: np.ndarray, v0: np.ndarray, v1: np.ndarray,
                         v2: np.ndarray) -> bool:
    a = v0 - p
    b = v1 - p
    c = v2 - p
