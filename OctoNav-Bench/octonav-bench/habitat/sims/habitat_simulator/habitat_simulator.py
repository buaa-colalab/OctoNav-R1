...
from habitat.core.simulator import (AgentState, DepthSensor, Observations,
                                    RGBSensor, SemanticSensor, Sensor,
                                    SensorSuite, ShortestPathPoint, Simulator,
                                    VisualObservation)
...
def overwrite_config(
    ...
    def if_config_to_lower(config):
        if isinstance(config, DictConfig):
            return {
                key.lower(): val
                for key, val in config.items() if isinstance(key, str)
            }
        else:
            return config
    ...
    for attr, value in config_from.items():
        ...
        if ignore_keys is None or low_attr not in ignore_keys:
            if hasattr(config_to, low_attr):
                ...
                raise NameError(f"""{low_attr} is not found on habitat_sim but
                    is found on habitat_lab config.
                    It's also not in the list of keys to ignore: {ignore_keys}
                    Did you make a typo in the config?
                    If not the version of Habitat Sim may not be compatible
                    with Habitat Lab version: {config_from}
                    """)
...
class HabitatSimDepthSensor(DepthSensor, HabitatSimSensor):
    ...
    def get_observation(
        ...
        if isinstance(obs, np.ndarray):
            ...
            obs = np.expand_dims(obs,
                                 axis=2)  # make depth observation a 3D array
        else:
            obs = obs.clamp(self.min_depth_value, self.max_depth_value
                            )  # type: ignore[attr-defined, unreachable]

            obs = obs.unsqueeze(-1)  # type: ignore[attr-defined]

        if self.normalize_depth:
            # normalize depth observation to [0, 1]
            obs = (obs - self.min_depth_value) / (self.max_depth_value -
...
def check_sim_obs(obs: Union[np.ndarray, "Tensor", None],
                  sensor: Sensor) -> None:
