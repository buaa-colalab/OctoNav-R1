...
"""
This module provides a diverse set of functional utilities for common
operations involving the Simulator and ManagedObjects including: object getters
from id and handle, geometric utilities, prepositional logic, region queries,
articulated object interactions, and more.
"""
...
def get_bb_for_object_id(
    ...
    :param ao_link_map: A pre-computed map from link object ids to their parent
    ArticulatedObject's object id.
    ...
    if obj is None:
        raise AssertionError(
            f"object id {obj_id} is not found, this is unexpected. "
            "Invalid/stale object id?")
...
def get_obj_size_along(
    ...
    """
    Uses object bounding box ellipsoid scale as a heuristic to estimate object
    size in a particular global direction.

    :param sim: The Simulator instance.
    :param object_id: The integer id of the object or link.
    :param global_vec: Vector in global space indicating the direction to
    approximate object size.
    :param ao_link_map: A pre-computed map from link object ids to their parent
    ArticulatedObject's object id.
    :return: distance along the specified direction and global center of
    bounding box from which distance was estimated.
    """
...
def size_regularized_bb_distance(
    ...
    """
    Get the heuristic surface-to-surface distance between two bounding boxes
    (regularized by their individual heuristic sizes).
    Estimate the distance from center to boundary along the line between bb
    centers. These sizes are then subtracted from the center-to-center distance
    as a heuristic for surface-to-surface distance.

    :param bb_a: local bounding box of one object
    :param bb_b: local bounding box of another object
    :param transform_a: local to global transform for the first object. Default
    is identity.
    :param transform_b: local to global transform for the second object.
    Default is identity.
    :param flatten_axis: Optionally flatten one axis of the displacement
    vector. This effectively projects the displacement. For example, index "1"
    would result in horizontal (xz) distance.
    ...
    # if object bounding boxes are significantly overlapping then distance may
    # be negative, clamp to 0
...
def size_regularized_object_distance(
    ...
    """
    Get the heuristic surface-to-surface distance between two objects
    (regularized by their individual heuristic sizes).
    Uses each object's bounding box to estimate the distance from center to
    boundary along the line between object centers. These object sizes are then
    subtracted from the center-to-center distance as a heuristic for
    ...
    assert (object_id_a != habitat_sim.stage_id
...
def bb_ray_prescreen(
    ...
    """
    Pre-screen a potential placement by casting rays in the gravity direction
    from the object center of mass (and optionally each corner of its bounding
    box) checking for interferring objects below.

    :param sim: The Simulator instance.
    :param obj: The RigidObject instance.
    :param support_obj_ids: A list of object ids designated as valid support
    surfaces for object placement. Contact with other objects is a criteria for
    placement rejection.
    :param ignore_obj_ids: A list of object ids which should be ignored in
    contact checks and raycasts. For example, the body of the agent placing an
    object.
    :param check_all_corners: Optionally cast rays from all bounding box
    corners instead of only casting a ray from the center of mass.
    :return: a dict of raycast metadata:
    "base_rel_height","surface_snap_point", "raycast_results"
    """
    ...
    for ix, key_point in enumerate(key_points):
        ...
        # NOTE: instead of explicit Y coordinate, we project onto any gravity
        # vector
        world_point_height = world_point.projected_onto_normalized(
            -gravity_dir).length()
        if (lowest_key_point is None
    ...
    # compute the relative base height of the object from its lowest bb corner
    # and COM
    base_rel_height = (
    ...
    # Warning: Bullet raycast on stage triangle mesh does NOT consider the
    # margin, so explicitly consider this here.
    margin_offset = 0
    ...
    surface_snap_point = (None if 0 not in support_impacts else
                          highest_support_impact + gravity_dir *
                          (base_rel_height - margin_offset))

    # return list of relative base height, object position for surface snapped
    # point, and ray results details
    return {
...
def snap_down(
    ...
    """
    Attempt to project an object in the gravity direction onto the surface
    below it.
    ...
    :param support_obj_ids: A list of object ids designated as valid support
    surfaces for object placement. Contact with other objects is a criteria for
    placement rejection. If none provided, default support surface is the
    stage/ground mesh (0).
    :param ignore_obj_ids: A list of object ids which should be ignored in
    contact checks and raycasts. For example, the body of the agent placing an
    object.
    :param dbv: Optionally provide a DebugVisualizer (dbv) to render debug
    images of each object's computed snap position before collision culling.
    :param max_collision_depth: The maximum contact penetration depth between
    the object and the support surface. Higher values are easier to sample, but
    result in less dynamically stabile states.
    :return: boolean placement success.

    Reject invalid placements by checking for penetration with other existing
    objects.
    If placement is successful, the object state is updated to the snapped
    location.
    If placement is rejected, object position is not modified and False is
    returned.

    To use this utility, generate an initial placement for any object above any
    of the designated support surfaces and call this function to attempt to
    snap it onto the nearest surface in the gravity direction.
    """
    ...
    bb_ray_prescreen_results = bb_ray_prescreen(sim,
                                                obj,
                                                support_obj_ids,
                                                ignore_obj_ids,
        ...
        for cp in cps:
            if (
                ...
                if cp.link_id_a > 0:
                    ...
                    links_to_obj_ids = {
                        v: k
                        for k, v in ao_a.link_object_ids.items()
                    }
                ...
                if cp.link_id_b > 0:
                    ...
                    links_to_obj_ids = {
                        v: k
                        for k, v in ao_b.link_object_ids.items()
                    }
                ...
                ) and (not (cp_obj_id_a in support_obj_ids
                            or cp_obj_id_b in support_obj_ids) or
                       (cp.contact_distance < (-1 * max_collision_depth))):
                    obj.translation = cached_position
                    return False
                    # print(f" Failure: contact in final position w/ distance =
                    # {cp.contact_distance}.")
                    # print(f" Failure: contact in final position with non
                    # support object {cp.object_id_a} or {cp.object_id_b}.")
...
def get_all_object_ids(sim: habitat_sim.Simulator) -> Dict[int, str]:
    """
    Generate a dict mapping all active object ids to a descriptive string
    containing the object instance handle and, for ArticulatedLinks, the link
    name.
    ...
    for _object_handle, ao in aom.get_objects_by_handle_substring("").items():
        ...
        for object_id, link_ix in ao.link_object_ids.items():
            object_id_map[object_id] = (ao.handle + " -- " +
                                        ao.get_link_name(link_ix))
...
def get_all_objects(
    ...
    """
    Get a list of all ManagedRigidObjects and ManagedArticulatedObjects in the
    scene.

    :param sim: The Simulator instance.
    :return: a list of ManagedObject wrapper instances containing all objects
    currently instantiated in the scene.
    """
...
def get_ao_link_id_map(sim: habitat_sim.Simulator) -> Dict[int, int]:
    """
    Construct a dict mapping ArticulatedLink object_id to parent
    ArticulatedObject object_id.
...
def get_ao_default_link(
    ...
    The "default" link is the one link which should be used if only one joint
    can be actuated. For example, the largest or most accessible drawer or
    door.

    :param ao: The ManagedArticulatedObject instance.
    :param compute_if_not_found: If true, try to compute the default link if it
    isn't found.
    :return: The default link index or None if not found. Cannot be base link
    (-1).
    ...
    The default link is determined by:
        ...
        - (if compute_if_not_found) - if not annotated, it is programmatically
        computed from a heuristic.

    Default link heuristic: the link with the lowest Y value in the bounding
    box with appropriate joint type.
    """
...
def get_obj_from_handle(
    ...
    """
    Get a ManagedRigidObject or ManagedArticulatedObject from its instance
    handle.

    :param sim: The Simulator instance.
    :param obj_handle: object instance handle for which ManagedObject is
    desired.
    :return: a ManagedObject or None
    """
...
def get_obj_transform_from_id(
    ...
    """
    Retrieve the local to global transform of the object or link identified by
    the object_id.
    ...
    if parent_obj.object_id == obj_id:
        ...
        return parent_obj.get_link_scene_node(
            parent_obj.link_object_ids[obj_id]).transformation


def get_global_keypoints_from_bb(
        aabb: mn.Range3D, local_to_global: mn.Matrix4) -> List[mn.Vector3]:
    """
...
def get_articulated_link_global_keypoints(
        ...
        link_index: int) -> List[mn.Vector3]:
    """
    Get global bb keypoints for an ArticulatedLink.

    :param object_a: The parent ManagedArticulatedObject for the link.
    :param link_index: The local index of the link within the parent
    ArticulatedObject. Not the object_id of the link.
    :return: A set of global 3D keypoints for the link.
    """
    link_node = object_a.get_link_scene_node(link_index)

    return get_global_keypoints_from_bb(link_node.cumulative_bb,
                                        link_node.absolute_transformation())
...
def get_global_keypoints_from_object_id(
    ...
    :param sim: The Simulator instance.
    :param object_id: The integer id for the object from which to extract
    keypoints.
    :param ao_link_map: A pre-computed map from link object ids to their parent
    ArticulatedObject's object id. If not provided, recomputed as necessary.
...
def object_keypoint_cast(
    ...
    """
    Computes object global keypoints, casts rays from each in the specified
    direction and returns the resulting RaycastResults.

    :param sim: The Simulator instance.
    :param object_a: The ManagedRigidObject from which to extract keypoints and
    raycast.
    :param direction: Optionally provide a unit length global direction vector
    for the raycast. If None, default to -Y.
    ...
    global_keypoints = get_global_keypoints_from_bb(object_a.aabb,
                                                    object_a.transformation)
    return [
...
def above(
    ...
    Concretely, 'above' is defined as: a downward raycast of any object
    keypoint hits the object below.
...
def within(
    ...
    Concretely, 'within' is defined as: a threshold number of opposing keypoint
    raycasts hit the same object.
    This function computes raycasts along all global axes from all keypoints
    and checks opposing rays for collision with the same object.

    :param sim: The Simulator instance.
    :param object_a: The ManagedRigidObject for which to query the 'within'
    set.
    :param max_distance: The maximum ray distance to check in each opposing
    direction (this is half the "wingspan" of the check). Makes the raycast
    more efficienct and realistically containing objects will have a limited
    size.
    :param keypoint_vote_threshold: The minimum number of keypoints which must
    indicate containment to qualify object_a as "within" another object.
    :param center_ensures_containment: If True, positive test of object_a's
    center keypoint alone qualifies object_a as "within" another object.
    ...
    for k_ix, keypoint in enumerate(global_keypoints):
        for a_ix in range(3):
            pos_ids = [
                hit.object_id for hit in sim.cast_ray(
    ...
    if center_ensures_containment:
        # initialize the list from keypoint 0 (center of bounding box) which
    ...
    containment_ids = containment_ids + [
        obj_id
        for obj_id in id_votes if id_votes[obj_id] > keypoint_vote_threshold
    ]
...
def ontop(
    ...
    """
    Get a list of all object ids or objects that are "ontop" of a particular
    object_a.
    Concretely, 'ontop' is defined as: contact points between object_a and
    object_b have vertical normals "upward" relative to object_a.
    This function uses collision points to determine which objects are resting
    on or contacting the surface of object_a.

    :param sim: The Simulator instance.
    :param object_a: The ManagedRigidObject or object id for which to query the
    'ontop' set.
    :param do_collision_detection: If True, a fresh discrete collision
    detection is run before the contact point query. Pass False to skip if a
    recent sim step or pre-process has run a collision detection pass on the
    current state.
    :param vertical_normal_error_threshold: The allowed error in normal
    alignment for a contact point to be considered "vertical" for this check.
    Functionally, if dot(contact normal, Y) <= threshold, the contact is
    ignored.
    :return: a list of integer object_ids for the set of objects "ontop" of
    object_a.
    """
    ...
    for cp in sim.get_physics_contact_points():
        ...
        obj_is_b = False
        if cp.object_id_a == object_a.object_id and (link_id is None or link_id
                                                     == cp.link_id_a):
        ...
        if contacting_obj_id is not None:
            contact_normal = (cp.contact_normal_on_b_in_ws
                              if obj_is_b else -cp.contact_normal_on_b_in_ws)
            if (mn.math.dot(contact_normal, yup)
...
def on_floor(
    ...
    """
    Checks if the object is heuristically considered to be "on the floor" using
    the navmesh as an abstraction. This function assumes the PathFinder and
    parameters provided approximate the navigable floor space well.
    NOTE: alt_pathfinder option can be used to provide an alternative navmesh
    sized for objects. This would allow objects to be, for example, under
    tables or in corners and still be considered on the navmesh.

    :param sim: The Simulator instance.
    :param object_a: The object instance.
    :param distance_threshold: Maximum allow-able displacement between current
    object position and navmesh snapped position.
    :param alt_pathfinder:Optionally provide an alternative PathFinder
    specifically configured for this check. Defaults to sim.pathfinder.
    :param island_index: Optionally limit allowed navmesh to a specific island.
    Default (-1) is full navmesh. Note the default is likely not good since
    large furniture objets could have isolated islands on them which are not
    the floor.
    :param ao_link_map: A pre-computed map from link object ids to their parent
    ArticulatedObject's object id.
    :return: Whether or not the object is considered "on the floor" given the
    configuration.
    """

    assert (not object_a.is_articulated), (
        "Object must be ManagedRigidObject, not implemented for "
        "ArticulatedObjects or links.")
    ...
    # use the object's heuristic size to estimate distance from the object
    # center to the navmesh in order to regularize the navigability constraint
    # for larger objects
...
def object_in_region(
    ...
    """
    Check if an object is within a region by checking region containment of
    keypoints.

    :param sim: The Simulator instance.
    :param object_a: The object instance.
    :param region: The SemanticRegion to check.
    :param containment_threshold: threshold ratio of keypoints which need to be
    in a region to count as containment.
    :param center_only: If True, only use the BB center keypoint, all or
    nothing.
    :param ao_link_map: A pre-computed map from link object ids to their parent
    ArticulatedObject's object id.
    :return: boolean containment and the ratio of keypoints which are inside
    the region.
    """
...
def get_object_regions(
    ...
    """
    Get a sorted list of regions containing an object using bounding box
    keypoints.
    ...
    :param ao_link_map: A pre-computed map from link object ids to their parent
    ArticulatedObject's object id.
    :return: A sorted list of region index, ratio pairs. First item in the list
    the primary containing region.
    """
...
def get_floor_point_in_region(
    ...
    :param island_index: The index of the navmesh island representing the
    active floor area. Default -1 is all islands. Should be set to the same
    island used for other navmesh operations in the application. For example,
    the largest indoor island for the scene.
    :param max_center_samples: The number of samples near the center point to
    attempt if applicable. This will be done first. <=0 skips center sampling.
    :param max_global_samples: The number of global navmesh samples to attempt
    if center point samples were unsuccessful. <=0 skips this step.
    :param quick_return: If True, the first valid sample will be returned
    instead of continuing to search for a better sample. Use this option when
    speed is more important than the quality or consistency.
    :return: The sampled floor point within the given region or None if a point
    could not be found.

    This method attempts to find a point in the region with maximum navmesh
    clearance by sorting candidates on `distance_to_closest_obstacle`.
    Because this method uses multiple sampling passes it is advised to use it
    in initialization and pre-processes rather than within an application loop.
    """
    ...
    if max_center_samples > 0:
        # get the center of the region's bounds and attempt to snap it to the
        # navmesh
        ...
        if not np.isnan(region_center_snap[0]):
            ...
            while attempts < max_center_samples:
                # get a point within 1 meter of the snapped region center if
                # possible
                ...
                if not np.isnan(sample[0]) and region.contains(sample):
                    ...
                    if navmesh_dist > best_navmesh_dist:
                        # found a valid point in a more "open" part of the
                        # region
                        best_sample = sample
...
def get_link_normalized_joint_position(
        ...
        link_ix: int) -> float:
    """
    Normalize the joint limit range [min, max] -> [0,1] and return the current
    joint state in this range.

    :param object_a: The parent ArticulatedObject of the link.
    :param link_ix: The index of the link within the parent object. Not the
    link's object_id.
    ...
    assert object_a.get_link_joint_type(link_ix) in [
        habitat_sim.physics.JointType.Revolute,
        habitat_sim.physics.JointType.Prismatic,
    ], (f"Invalid joint type '{object_a.get_link_joint_type(link_ix)}'. "
        "Open/closed not a valid check for multi-dimensional or fixed joints.")
    ...
    n_pos = (joint_pos - limits[0][joint_pos_ix]) / (limits[1][joint_pos_ix] -
                                                     limits[0][joint_pos_ix])
    return n_pos
...
def set_link_normalized_joint_position(
    ...
    """
    Set the joint's state within its limits from a normalized range [0,1] ->
    [min, max]
    ...
    assert (normalized_pos <= 1.0 and normalized_pos >= 0), (
        "values outside the range [0,1] are by definition beyond the joint "
        "limits.")
...
def link_is_open(
    ...
    We assume that joint limits define the closed state (min) and open state
    (max).

    :param object_a: The parent ArticulatedObject of the link to check.
    :param link_ix: The index of the link within the parent object. Not the
    link's object_id.
    :param threshold: The normalized threshold ratio of joint ranges which are
    considered "open". E.g. 0.8 = 80%
...
def close_link(object_a: habitat_sim.physics.ManagedArticulatedObject,
               link_ix: int) -> None:
    """
    Set a link to the "closed" state. Sets the joint position to the minimum
    joint limit.

    TODO: does not do any collision checking to validate the state or move any
    other objects which may be contained in or supported by this link.
...
def open_link(object_a: habitat_sim.physics.ManagedArticulatedObject,
              link_ix: int) -> None:
    """
    Set a link to the "open" state. Sets the joint position to the maximum
    joint limit.
...
def bb_next_to(
    ...
    """
    Check whether or not two bounding boxes should be considered "next to" one
    another.
    ...
    :param transform_b: local to global transform for the second object.
    Default is identity.
    :param hor_l2_threshold: regularized horizontal L2 distance allowed between
    the objects' centers.
    :param vertical_padding: vertical distance padding used when comparing
    bounding boxes. Higher value is a looser constraint.
    :return: Whether or not the objects are heuristically "next to" one
    another.

    Concretely, consists of two checks:
        1. assert overlap between the vertical range of the two bounding boxes.

        2. regularized horizontal L2 distance between object centers.
        Regularized in this case means projected displacement vector is
        truncated by each object's heuristic size.
    ...
    if (highest_height_a + vertical_padding) < lowest_height_b or (
            highest_height_b + vertical_padding) < lowest_height_a:
        return False

    if (size_regularized_bb_distance(bb_a,
                                     bb_b,
                                     transform_a,
                                     transform_b,
                                     flatten_axis=1) > hor_l2_threshold):
        return False
...
def obj_next_to(
    ...
    """
    Check whether or not two objects should be considered "next to" one
    another.
    ...
    :param hor_l2_threshold: regularized horizontal L2 distance allow between
    the objects' centers. This should be tailored to the scenario.
    :param vertical_padding: vertical distance padding added when comparing the
    object's bounding boxes. Higher value is a looser constraint.
    :param ao_link_map: A pre-computed map from link object ids to their parent
    ArticulatedObject's object id.
    :return: Whether or not the objects are heuristically "next to" one
    another.
    ...
    Concretely, consists of two checks:
        ...
        2. regularized horizontal L2 distance between object centers must be
        less than a threshold. Regularized in this case means displacement
        vector is truncated by each object's heuristic size.
    """
...
def point_to_tri_dist(point: np.ndarray,
                      triangles: np.ndarray) -> Tuple[float, np.ndarray]:
    """
    Compute the minimum distance between a 3D point and a set of triangles
    (e.g. a triangle mesh) and return both the minimum distance and that
    closest point.
    Uses vectorized numpy operations for high performance with a large number
    of triangles.
    Implementation adapted from a StackOverflow discussion on closest-point
    projection of a 3D point to a triangle.
    Algorithm is a vectorized form of the geometrictools
    DistancePoint3Triangle3 reference.

    :param point: A 3D point.
    :param triangles: An nx3x3 numpy array of triangles. Each entry of the
    first axis is a triangle with three 3D vectors, the vertices of the
    triangle.
    :return: The minimum distance from point to triangle set and the closest
    point on the surface of any triangle.
    """
    ...
    with np.errstate(all="ignore"):
        ...
        # now extract the closest point on the mesh and minimum distance for
        # return
...
def get_obj_receptacle_and_confidence(
    ...
    """
    Heuristic to match a potential placement point with a Receptacle and
    provide some confidence.

    :param bottom_point: The bottom center point of the object or equivalent
    (e.g the candidate raycast point for placement)
    :param obj: The ManagedRigidObject for which to find a Receptacle match.
    :param support_surface_id: The object_id of the intended support surface
    (rigid object, articulated link, or stage_id). If not provided, a downward
    raycast from object center will determine the first hit as the support
    surface.
    :param obj_bottom_location: The optional location of the candidate bottom
    point of the object. If not provided, project the object center to the
    lowest extent.
    :param max_dist_to_rec: The threshold point to mesh distance for an object
    to be matched with a Receptacle.
    :param candidate_receptacles: Optionally provide a dict (unique_name to
    Receptacle) of candidate Receptacles for matching. If not provided, uses
    sim.receptacles.
    :param island_index: Optionally provide an island_index for identifying
    navigable floor points. Default is full navmesh.
    :return: Tuple containing: (1): list of receptacle strings: "floor,region",
    Receptacle.unique_name, or None (2): a floating point confidence score
    [0,1] (3): a message string describing the results for use in a UI tooltip

    When using this util for candidate placement with raycasting (e.g. HitL):
    provide 'support_surface_id' and 'obj_bottom_location' overrides from the
    raycast.
    When using this util for assessing current object state (e.g. episode
    evaluation): leave 'support_surface_id' and 'obj_bottom_location' as
    default.
    """
    # collect information about failure of the function
    info_text = ""

    # get the center point of the object projected on the local bounding box
    # size in the gravity direction
    ...
    obj_bottom_point = (center + grav_vector * dist if obj_bottom_location
                        is None else obj_bottom_location)

    # find a support surface if one was not provided
    if support_surface_id is None:
        raycast_results = sim.cast_ray(habitat_sim.geo.Ray(
            center, grav_vector))
    ...
    # in case we cannot match the object|point to a Receptacle object, we'll
    # try to validate that it could be "on the floor"
    ...
    if support_surface_id == habitat_sim.stage_id:
        ...
        matching_recs = [
            rec for u_name, rec in candidate_receptacles.items()
        ...
        if len(matching_recs) == 0:
            ...
            # define an epsilon distance similarity threshold for determining
            # two receptacles are equidistant (e.g. overlapping)
            same_dist_eps = 0.01
            min_indices = []
            min_dist: float = None
            for ix, dist in enumerate(dist_to_recs):
                if dist < max_dist_to_rec:
                    if min_dist is None or dist < (min_dist - same_dist_eps):
                        ...
                        # this rec is nearly equidistant to the current best
                        # match
            ...
            if len(min_indices) > 0:
                ...
                info_text = (
                    "Point is too far from a valid Receptacle on the support "
                    "surface.")

    # If we made it this far, matching to a Receptacle object failed.
    # Now for some cases, check if the point is navigable and if so, try
    # matching it to a region
    if fallback_to_floor_matching:
        # NOTE: using navmesh snapping to a point within 10cm horizontally as a
        # heuristic for "on the floor"
        ...
        if (obj_bottom_point - snap_point).length() < 0.1:
            ...
            if len(point_regions) > 0:
                ...
                region_name = sim.semantic_scene.regions[point_regions[0]
                                                         [0]].id
            else:
