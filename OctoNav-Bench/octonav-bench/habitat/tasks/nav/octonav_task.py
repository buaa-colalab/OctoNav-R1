...
from habitat.core.embodied_task import EmbodiedTask, Measurements, Measure
...
try:
    from habitat.datasets.octonav.octonav_dataset import (
        OctoNavDatasetV1,
    )
except ImportError:
    pass
...
class OctoObjectGoalSensor(Sensor):
    ...
    def get_observation(
        ...
        **kwargs: Any,
        ...
        if self.config.goal_spec == "TASK_CATEGORY_ID":
            return np.array(
                [self._dataset.category_to_task_category_id[episode.dataset_name][category_name]],
                dtype=np.int64,
            )
...
class OctoInstanceImageGoalHFOVSensor(Sensor):
    ...
    def get_observation(
        self, *args: Any, episode: OctoNavEpisode, **kwargs: Any
    ) -> np.ndarray:
        if len(episode.goals) == 0:
            logger.error(
                f"No goal specified for episode {episode.episode_id}."
...
class OctoNavInstructionSensor(Sensor):
    ...
    def _get_observation(
        ...
        **kwargs
        ...
        if episode.task_name == 'PointNav':
            agent_position = np.array(episode.start_position).astype(np.float32)
            rotation_world_agent = quaternion.quaternion(*episode.start_rotation[::-1])
            goal_position = np.array(episode.goals[0].position, dtype=np.float32)
            ...
            if phi > 0:
                rotate_description = f'a clockwise angle of {degree:.2f} degrees'
            else:
                rotate_description = f'an anti-clockwise angle of {degree:.2f} degrees'
            return {
                "text": f"Move towards the target point located at a distance {rho:.2f}m and {rotate_description}; after that, stop. ",
            }
        elif episode.task_name == 'ObjectNav':
            return {
                "text": f'Find the {episode.object_category} and then head straight to the {episode.object_category}; after that, stop. ',
            }
        elif episode.task_name == 'ImageNav':
            return {
                "text": 'Align your trajectory using the reference image {ImageNav} to approach the scene matching the visual perspective; after that, stop.',
                "ImageNav": self.get_imagegoal(episode)
            }
        elif episode.task_name == 'InstanceImageNav':
            return {
                "text": 'Locate the object instance in the reference image {InstanceImageNav} and verify its presence in the current view; after that, stop.',
            ...
            for task_episode in episode.task_episodes:
                if task_episode.task_name == 'ImageNav':
                    ...
                    data_dict['InstanceImageNav'] = self.get_instanceimagegoal(task_episode)
            return data_dict
        else:
            return None
    ...
    def get_observation(
        ...
        **kwargs
        ...
        self._current_instruction = self._get_observation(episode=episode, **kwargs)
        self._current_episode_id = episode_uniq_id
        return self._current_instruction
    ...
    def reset_metric(self, episode, *args: Any, **kwargs: Any):
        ...
        for task_episode in episode.task_episodes:
            ...
            if self._distance_to[task_name] == "VIEW_POINTS":
                ...
                logger.error(
                    f"Non valid distance_to parameter was provided: {self._distance_to[task_name]}"
                )
...
class OctoNavTaskSuccess(Measure):
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        ...
        for task_episode in episode.task_episodes:
            self.success_distance.append(self._success_distance[task_episode.task_name])
    ...
    def update_metric(
        ...
        distance_to_target = task.measurements.measures[
            OctoNavDistanceToGoal.cls_uuid
        ].get_metric()

        self._metric = [1.0 if distance_to_target[i] < self.success_distance[i] else 0.0 for i in range(self.task_num)]
...
class OctoNavSuccess(Measure):
    ...
    def update_metric(
        ...
        task_success = task.measurements.measures[
            OctoNavTaskSuccess.cls_uuid
        ].get_metric()
        
        if self.current_task < self.task_num and task_success[self.current_task]:
            self.current_task += 1
        self._metric = [1.0] * self.current_task + [0.0] * (self.task_num - self.current_task)
...
class OctoNavOracleSuccess(Measure):
    ...
    def update_metric(
        ...
        self.succeed = [max(self.succeed[i], task_success[i]) for i in range(self.task_num)]
...
class OctoNavSPL(Measure):
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        task.measurements.check_measure_dependencies(
            self.uuid, [OctoNavSuccess.cls_uuid]
        )
    ...
    def update_metric(
        ...
        cur_success_task = task.measurements.measures[OctoNavSuccess.cls_uuid].current_task
        ...
        while self.success_task < cur_success_task:
            i = self.success_task
            self.record_metric[i] = self._start_end_episode_distance[i] / max(self._start_end_episode_distance[i], self._agent_episode_distance)
            self.success_task += 1
...
class OctoNavLocalSPL(Measure):
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        ...
        for i in range(1, self.task_num):
            self._start_end_episode_distance.append(episode.shortest_path[i] - episode.shortest_path[i-1])
        self.success_task = 0
    ...
    def update_metric(
        ...
        while self.success_task < cur_success_task:
            ...
            self.record_metric[i] = self._start_end_episode_distance[i] / max(self._start_end_episode_distance[i], dis)
...
class OctoNavTopDownMap(Measure):
    ...
    def _draw_goals_positions(self, episode, color=maps.MAP_TARGET_POINT_INDICATOR):
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        ...
        self.cur_task = 0
        self.task_names = [task_episode.task_name for task_episode in episode.task_episodes]

        if hasattr(episode, "goals"):
            # draw source and target parts last to avoid overlap
            for task_episode in episode.task_episodes:
                # self._draw_goals_view_points(task_episode)
                self._draw_goals_positions(task_episode, maps.TASK_COLOR[task_episode.task_name])
    ...
    def update_metric(self, episode, task, *args: Any, **kwargs: Any):
        ...
        while self.cur_task + 1 < len(success) and success[self.cur_task] == 1.0:
            self.cur_task += 1
        self._metric = {
...
class OctoNavTask(NavigationTask):
    ...
    def get_multi_metrics(self):
        distance_to_goal = self.measurements.measures[OctoNavDistanceToGoal.cls_uuid].get_metric()
        success = self.measurements.measures[OctoNavSuccess.cls_uuid].get_metric()
        oracle_success = self.measurements.measures[OctoNavOracleSuccess.cls_uuid].get_metric()
        final_success = self.measurements.measures[OctoNavFinalSuccess.cls_uuid].get_metric()
        spl = self.measurements.measures[OctoNavSPL.cls_uuid].get_metric()
        localspl = self.measurements.measures[OctoNavLocalSPL.cls_uuid].get_metric()
        success_and_stop = success[-1] * final_success
        result = {'OctoNav': {
        ...
        for idx, task_episode in enumerate(self.multitask_episode.task_episodes):
            task_name = task_episode.task_name
        ...
        if OctoNavTopDownMap.cls_uuid in self.measurements.measures:
            result['OctoNav']['top_down_map'] = self.measurements.measures[OctoNavTopDownMap.cls_uuid].get_metric()
        return result
