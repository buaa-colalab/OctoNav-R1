...
class PointGoalSensor(Sensor):
    r"""Sensor for PointGoal observations which are used in PointGoal
    Navigation.
...
class ImageGoalSensor(Sensor):
    r"""Sensor for ImageGoal observations which are used in ImageGoal
    Navigation.
    ...
    def __init__(self, *args: Any, sim: Simulator, config: "DictConfig",
                 **kwargs: Any):
        self._sim = sim
        sensors = self._sim.sensor_suite.sensors
        rgb_sensor_uuids = [
            uuid for uuid, sensor in sensors.items()
            if isinstance(sensor, RGBSensor)
        ]
        if len(rgb_sensor_uuids) != 1:
            raise ValueError(f"ImageGoalNav requires one RGB sensor, \
{len(rgb_sensor_uuids)} detected")
    ...
    def _get_pointnav_episode_image_goal(self, episode: NavigationEpisode):
        ...
        hash_obj = hashlib.sha256(str(episode.episode_id).encode("utf-8"))
...
class ProximitySensor(Sensor):
    ...
    def __init__(self, sim, config, *args: Any, **kwargs: Any):
        self._sim = sim
        self._max_detection_radius = getattr(config, "max_detection_radius",
                                             2.0)
        super().__init__(config=config)
    ...
    def get_observation(self, observations, *args: Any, episode,
                        **kwargs: Any):
...
class Success(Measure):
    ...
    def update_metric(self, episode, task: EmbodiedTask, *args: Any,
        ...
        distance_to_target = task.measurements.measures[
            DistanceToGoal.cls_uuid].get_metric()
    ...
    def _draw_goals_positions(self, episode):
        if self._config.draw_goal_positions:
            for goal in episode.goals:
                if self._is_on_same_floor(goal.position[1]):
                    try:
                        self._draw_point(goal.position,
    ...
    def update_metric(self, episode: NavigationEpisode, *args: Any,
        ...
        if self._previous_position is None or not np.allclose(
            ...
            if self._distance_to == "POINT":
                ...
                logger.error(f"Non valid distance_to parameter was \
provided: {self._distance_to}")
...
class NavigationMovementAgentAction(SimulatorTaskAction):
    ...
    def _move_camera_vertical(self, amount: float):
        ...
        sensor_names = list(
            self._sim.agents[0]._sensors.keys())  # type: ignore
        for sensor_name in sensor_names:
            sensor = self._sim.agents[0]._sensors[
                sensor_name].node  # type: ignore
...
class VelocityAction(SimulatorTaskAction):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self._allow_sliding = \
            self._sim.config.sim_cfg.allow_sliding  # type: ignore
