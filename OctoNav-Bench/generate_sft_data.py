...
if __name__ == '__main__':
    ...
    config = habitat.config.get_config_and_task(
        ...
        overrides=[f'habitat.environment.iterator_options.max_scene_repeat_steps=-1']
    )
    ...
    with habitat.Env(config=config) as env:
        ...
        for _ in pbar:
            ...
            for idx, step in enumerate(env.current_episode.steps):
                ...
                if np.sum(np.abs(np.array(cur_position) - np.array(position))) > 0.2:
                    assert False, 'position error'
                if np.sum(np.abs(np.array(cur_rotation) - np.array(rotation))) > 0.2:
                    assert False, 'rotation error'
                observations = env.step(action)
            if env.task.measurements.measures['octonav_success'].get_metric()[-1] != 1.0:
